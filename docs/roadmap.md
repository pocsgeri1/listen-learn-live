...
- **OG Map hyper-zoom — visual tuning pass (2026-07-04)** — v2.25 shipped all 24 nodes as clickable hyper-zoom targets, but zoom-target scale values and Grit's tightly-packed sub-node hit-circle sizing were tuned without a live browser preview. Worth one quick pass in-browser (desktop + mobile) to confirm nothing clips or feels off before calling this fully done.
