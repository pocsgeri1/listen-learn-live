...
- **Chronological ordering (deferred, 2026-07-03)** — Gergely flagged a chronological-ordering feature during the same session that shipped Phases 2/5-10/12-13 below, then decided to skip execution for now: not started, no code touched. Revisit when prioritised.
...
- v3.41 ✅ — 2026-08-10 — **Nav redesign, signup modal + email gate, light mode overhaul (Crisp Paper + forest green), I Feel Epic → Home panel.** Nav right side: removed epic button + concept count, added theme pill (icon+label) + signup button, editorial text links with dot separators in center. Email-capture modal with founding member banner, aspirational copy, POST to Brevo `/api/subscribe`. Soft email gate: one free use per AI feature then modal, `ep_unlocked`/`ep_used_*` in localStorage. Light mode: full palette swap to `#faf8f4` base + `#3d6b52` forest green accent across all 257 `[data-theme="light"]` rules + SVG hairline. Mood row with 4 emoji buttons added to Home dashboard panel.
...
- v2.24 ✅ — 2026-07-03 — **Site performance overhaul.** Fixed the largest measured CLS contributor (0.2483, `#themeToggle` pushed sideways by `.nav-epic-standalone`'s hover animation) by switching to opacity/transform-only. Chunked the episode drawer's card-grid render (column-per-`requestAnimationFrame`) to fix 288-350ms drawer-open INP. Fixed a forced-layout-reflow regression in intel-pill hover popovers (was re-measuring on every hover since v2.20). Sped up card-flip (0.62s→0.4s) and decoupled its hover shadow from the flip transition. Reduced `backdrop-filter` blur cost on two overlays. Capped the Corner constellation canvas loop to ~30fps and repositioned it lower (was overflowing into the search UI). Added a standing "performance defaults" rule set to `cowork-default-instructions.md` Step 2a for all future build sessions. Confirmed podcast thumbnails are NOT a hidden drawer-open cost (no `<img>` tags in the card template; hero backgrounds are already lightweight).
- v2.23 ✅ — 2026-07-03 — **OG Map nav fix, extract.html editorial tooling, Spark feedback-row bug, Corner mobile fixes, Sparring persistence.** OG Map panel no longer covers the nav (live nav-height measurement). `extract.html`: word/char counters on all fields incl. Summary/Vocab, real back/forward version history + restore-original, stronger Episode Intel divider, new sticky rules sidebar (concept field rules / Episode Intel summary rules, cross-fades on scroll, never touches the 880px column). Spark "Did it land?" row fixed — was appearing after 0.4s instead of 8s due to a competing unconditional CSS animation. Corner submit button shrinks to 🥊-only on mobile (≤700px), desktop untouched. Corner Sparring results now persist per concept via localStorage, feeding Situations tab's Revisit for free. Chronological ordering and OG Map light mode scoped and deferred/skipped this session — see below and "Next up."
- v2.22 ✅ — 2026-07-02 — **Make.com → GitHub Actions migration, publish step: live and confirmed.** `tools/publish-approved.js` + `.github/workflows/publish-approved.yml` (manually-triggered GitHub Action) replaces Make.com's "APPROVED → publish" scenario — fetches Airtable APPROVED rows, POSTs to `/api/publish-batch.js`, writes results back. Confirmed working end-to-end on a live test episode. Hit and fixed a recurring Airtable 403 (`INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND`) by rotating the `AIRTABLE_API_KEY` GitHub secret to a fresh PAT — see build-journal Lesson 49 (2026-07-02). `architecture.md` and `pending-decisions.md` updated to match; old Make.com scenario to be paused now that this is confirmed.
...
- v2.17 ✅ — 2026-07-02 — **Extraction prompt reconciliation + quality-drift guardrails.** Ported missing `concept-rewrite-prompt.md` rules (cross-field image check, full-field anti-slop scope) into `extract.html` and `api/extract-concepts.js`. Moved EPISODE INTEL section to the end of the prompt (was diluting concept rules mid-prompt). Removed stale LLM-assigned `curated_collection_ids` instruction from `extract-concepts.js`. Added quality-drift guardrails: continuous per-concept self-check (not end-of-batch), explicit quality-over-quantity rule, extended thinking enabled on the main extraction call for real self-check scratch space. See build-journal 2026-07-02 for the full reasoning.
...
- v2.15c ✅ — 2026-07-01 — **Desktop/mobile copy forking (Founder + OG text).** New `.fc-desktop-only`/`.fc-mobile-only` utility classes let specific lines genuinely diverge per breakpoint instead of compromising on one string. Masturbation line, Podcast-summaries bullet, impostor-syndrome line, Spark bullet, Next-up separator symbols, and the closing "Stay tuned" lines all fork desktop vs mobile now. Closing lines redesigned: 3 rows on desktop, 2 on mobile. "epistemic" bolded+italicized on top of its accent color (both breakpoints).

- v2.15b ✅ — 2026-07-01 — **Copy tweaks + desktop drawer cat-filter bug fix.** Founder Copy masturbation line: no more forced break on desktop, smaller font on mobile so it wraps naturally. OG bullets restyled as plain stacked lines (was reading like a table). Bullet copy tightened, Listen〱Read〱Write〱Speak〱Grow bolded, map hint line rewritten with an "Art of Impossible" callout, "epistemic" accented gold, og-kicker/expand-label headers weight-matched. Fixed two desktop-only drawer bugs: scan view was hiding category pills entirely and shoving the view buttons left (mobile-only CSS rule had no media guard); All-cards view was disabling clicks on cat pills even though they're wired to scroll-to-category. Both fixed desktop-only, mobile drawer untouched.
...
- v2.15a ✅ — 2026-07-01 — **Copy polish pass + OG Map fullscreen reverted.** Founder Copy line-wrap now forced via `<br>` (not width-dependent `&nbsp;` wrap), "nevermind" closing line cut. OG text: impostor-syndrome line bolded/italicized for emphasis, "Right now" paragraph converted to 3 on-brand bullets (podcasts/Corner/Spark, no internal "intelligence pills" language exposed), "Next up" line rewritten with 〱 separators, "Stay tuned" closer now bold. OG Map fullscreen/rotate feature (shipped in v2.15) fully reverted — didn't hold up in testing, map is back to zoom/pan/reset only.
