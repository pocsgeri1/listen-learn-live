...
- **SEO Session 3 — Category pages** — `/category/[name].html` × 14. One page per category listing all concept links. Breadcrumb JSON-LD. Extend `generate-static-pages.js` or write separate script.
- **SEO Session 4 — Redirect helper** — `tools/update-seo-redirects.js` diffs old/new `concepts.json` for term renames → generates 301 entries for `vercel.json`.
