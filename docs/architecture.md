...
- `lll_folders_v1` — Array of user-created folder objects (added v3.49). Each entry: `{ id: "f_{timestamp}", name, icon, color, emoji, conceptIds: integer[], vocabWords: object[], noteIds: string[], canvasLayout: object|null, canvasItems: array, createdAt: ms, updatedAt: ms, pinned: bool }`. `id` generated as `"f_" + Date.now()`. `canvasLayout` is `{ [conceptId]: {x, y} }`, pruned to current members on save. `canvasItems` holds freeform canvas elements (note / label / vocab / ytclip / link, added v3.54).
  - **`vocabWords` holds objects, not strings, since v3.56:** `{ word, definition, category, colId }`. Written by `_vocabFolderToggle`. Any reader must guard with `(w && typeof w === 'object') ? w.word : w` — pre-v3.56 folders may still contain raw strings, and v3.57a fixed three readers that assumed strings and made folders vanish. Do not "clean up" this guard.
  - CRUD: `_folderCreate`, `_folderAddConcept`, `_folderRemoveConcept`, `_folderAddVocab`, `_folderRemoveVocab`, `_folderDelete`.
  - **V3 extends this object** with `viewport`, `connections[]`, `captureIds[]`, `draftIds[]`, `coverIds[]`, `tagIds[]`, `chatThreadId`, `schema` — all additive and optional. See `v3-architecture.md` §9.3.
- **`lll_saved_v1` does not exist.** It is referenced in older planning docs and in the V3 strategy brief, but there is no such key anywhere in `index.html`. Saved/mastered concepts live in `lll_mastered_ts_v1` as `{ [conceptId]: timestampMs }` (21 call sites). Never introduce `lll_saved_v1`.
- `lll_mastered_ts_v1` — JSON object `{ [conceptId]: timestamp_ms }`. Written by `toggleMaster()` on each master action (not on un-master). Also written by `_ccToggleNote` blur handler (v3.17): if a non-empty note is saved on an unsaved concept, the concept is auto-saved here with the current timestamp. Note deletion does NOT remove the entry. Read by My Library Saved tab to sort by recency. *Added v3.02.*
...
- `vocab_vault` — array of `{ word, definition, timestamp_seconds, category, category_alt }`. Target: 35–40 entries per episode (increased from 20–25 in v2.46, July 2026). `category` is exactly one of the 9 register-based vocab categories (see `docs/vocab-categories.md`). `category_alt` is always `null` during extraction — reserved for a future dedicated dual-category pass. The UI previews 5–7 by default; the full list is stored so the preview count can change without re-extraction. **Backfill:** run `tools/categorize-vocab.js` to add `category` to existing entries extracted before v2.46.

**Enrichment fields (added v2.46 — generated by `tools/generate-episode-enrichment.js`):**
- `difficulty_level` — `1` (accessible) / `2` (some background needed) / `3` (dense/specialist)
- `tone` — `"challenging"` / `"warm"` / `"practical"` / `"philosophical"` / `"confrontational"`
- `guest_field` — string, primary domain of guest (e.g. `"neuroscience"`, `"marketing"`)
- `key_quotes` — array of 2–3 short verbatim-ish memorable quotes
- `core_claim` — one sentence: the single thesis the episode orbits
- `episode_type` — `"interview"` / `"solo"` / `"debate"` / `"panel"`
- `actionability_score` — `1` (pure ideas) / `2` (some takeaways) / `3` (concrete steps)
- `evergreen` — boolean. Will this episode be as valuable in 5 years?
- `controversy_flag` — boolean. Do reasonable people strongly disagree with the main argument?
- `guest_return` — boolean. Auto-calculated from `collections.json` (no API). True if any guest appears in 2+ episodes.
- `concept_density` — integer. Auto-calculated: concept count for this episode.
- `episode_length_minutes` — integer or null. Manually backfilled. Used to compute true concepts-per-minute density in future.
...
**Generation:** `tools/generate-episode-intel.js` — intel fields (summary, vocab, etc.). `tools/generate-episode-enrichment.js` — enrichment fields (difficulty, tone, etc.). Both call `claude-haiku-4-5-20251001`, write to `episode_meta.json` after each episode (crash-safe). `extract.html` (epistemic-tools repo) has a browser-side intel panel for per-episode generation with style override.
