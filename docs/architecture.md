...
- `lll_folders_v1` — Array of user-created folder objects (added v3.49). Each entry: `{ id: "f_{timestamp}", name, icon, color, conceptIds: integer[], vocabWords: string[], noteIds: string[], canvasLayout: object|null, createdAt: ms, updatedAt: ms, pinned: bool }`. `id` generated as `"f_" + Date.now()`. `canvasLayout` reserved for Phase 4 canvas. `vocabWords` cross-reference `lll_lexicon_v1` by lowercase word string. CRUD: `_folderCreate`, `_folderAddConcept`, `_folderRemoveConcept`, `_folderAddVocab`, `_folderRemoveVocab`, `_folderDelete`.
