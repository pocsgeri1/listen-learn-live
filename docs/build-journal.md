...
**Lesson 65 — SVG with `@import url(...)` inside a `<style>` block is not well-formed XML and will cause cairosvg/lxml to fail with `not well-formed (invalid token)`.** When generating the OG image, the first attempt used cairosvg to convert an SVG to PNG. The SVG had a `<style>` block with `@import url('https://fonts.googleapis.com/...')` — valid CSS, but invalid XML (the parentheses break XML tokenization). cairosvg failed immediately. Fix: use Python Pillow to draw the image programmatically instead of via SVG → PNG conversion. **Lesson: if an SVG uses CSS `@import` for web fonts, skip cairosvg entirely and build the image programmatically with Pillow — it's faster and has no XML constraints.**

**Lesson 66 — `json.dump()` without a trailing newline write creates a "no newline at end of file" git diff that looks like a change but contains no actual content change.** A patch script that read and rewrote `concepts.json` via `json.dump()` (no `f.write('\n')` after) left a one-character diff — the missing trailing newline. `git restore` was blocked by sandbox permissions. Fixed by reopening the file in binary append mode and writing `b'\n'` only if the last byte wasn't already `\n`. **Lesson: always end a `json.dump()` to a file with `f.write('\n')` to match the original file's trailing newline and avoid a spurious diff.**

### 2026-07-04 — v2.25a: OG Map spotlight modal rebuild + content restructuring

**Lesson 61 — An SVG `<foreignObject>` nested inside an element with a CSS `transform: scale()` applied inherits that scale for layout purposes, which reads as blurry raster-stretched text, not a rendering bug you can patch with CSS.** v2.25's content card was a foreignObject inside `#ogMapInner`, the element the pan/zoom system scales 2.2-3.6× on activation. This single choice caused all three bugs Gergely caught on first live test: bleed-through (decorative map text scales up and ghosts through the card's translucent background), fuzzy text (HTML laid out at 1x then stretched by the ancestor's scale), and cut-off text (the foreignObject's fixed height had no give for stacked paragraphs). **Lesson: any HTML content that needs to render at a fixed, undistorted size must live outside any ancestor with a CSS `transform` applied — an SVG foreignObject is not an escape hatch from an ancestor's transform, it inherits it like any other descendant.**

**Lesson 62 — An ancestor with `overflow: hidden` (or `overflow-x: hidden`, which forces the other axis to compute as `auto`) clips `position: fixed` descendants based on DOM ancestry, not containing-block chain — a second gotcha independent of the transform issue above.** Even after deciding to move the card outside `#ogMapInner`'s transform, it still couldn't live inside `.og-map-wrap`, because that element has `overflow-x: hidden` for an unrelated reason (containing the map's horizontal scroll). Fixed by placing `#ogSpotlight` as a sibling of `.og-map-wrap` entirely, confirmed by checking the ancestor chain's CSS for both `transform` and `overflow` before picking the insertion point, then verifying by walking the actual DOM output (div-open/close counts between the two insertion candidates) rather than assuming. **Lesson: when placing a `position: fixed` overlay, audit every ancestor between it and its old location for both `transform` and non-`visible` `overflow` — either one alone breaks fixed positioning, and they're easy to check for independently but easy to only check for one of.**

**Lesson 63 — A regex-based sentence splitter that requires a terminator to be followed by whitespace will silently drop entire leading chunks of text when a quoted question mark appears mid-sentence, and this kind of bug won't show up unless you check length/content round-trip against the real dataset, not just a few spot-checked examples.** `ogSplitSentences`'s first version used `/[^.!?]+[.!?]+(?:\s+|$)/g`. Two real `ogmap.json` entries embed a quoted rhetorical question mid-sentence (`ask 'how will I feel about this decision in 5 years?' — write the answer down.`) where the inner `?` is immediately followed by a closing quote, not whitespace — the regex can't match a boundary there, so it skips forward silently and only captures the trailing half of the string, dropping the entire lead sentence. This wasn't caught by eyeballing a handful of examples during design; it was caught by scripting a round-trip length check (split, rejoin, compare non-whitespace character count to the original) against all 72 real insight/why/protocol fields in the dataset before shipping. Fixed by allowing an optional closing quote/apostrophe between the terminator and the required whitespace/end. **Lesson: when a text-processing regex will run against real production content, verify it against the *entire* real dataset programmatically (a round-trip check is cheap to write) rather than a few manually-inspected examples — edge cases that "look rare" show up reliably at n=72 and won't be visible in a 5-example spot check.**

### 2026-07-04 — v2.25: OG Map hyper-zoom rebuild

**Lesson 58 — When a feature needs "zoom toward a point, then smoothly settle," check whether an existing pan/zoom system already exposes that primitive before building a second animation system.** The OG Map hyper-zoom (click a node → zoom in → content card appears in-place) could have been built as an independent SVG `viewBox` rAF animation. Instead, the existing v2.14c CSS-transform pan/zoom closure (`scale`/`ox`/`oy`/`apply()`, already driving wheel-zoom and drag-pan) already computes exactly the transform needed to center-and-scale toward any point — it just needed two new functions (`ogZoomToNode`/`ogZoomReset`) built around a shared `animateTransform()` helper that temporarily overrides the CSS transition duration for a custom-length animated zoom. One code path, nothing new to keep in sync with the existing pan/zoom behavior, and no new dependency. **Lesson: before adding a parallel animation/state system for a new interaction, check whether the primitive already exists in a system that's doing something adjacent — reusing it is almost always lower-risk than a clean-room build.**

**Lesson 59 — Version-number collisions happen across sessions when code comments are written before the changelog entry lands.** The OG Map hyper-zoom code was originally drafted with `v2.24` comment tags mid-session, but by the time this session resumed and shipped, v2.24 had already been claimed by an unrelated performance-overhaul release logged earlier the same day. Caught during the doc-update pass by cross-checking the changelog's latest version against the code's own comment tags before writing the new entry — renamed the 3 affected comments to `v2.25` to match. **Lesson: when resuming a build session after a gap, always check `changelog.md`'s top entry for the current version number before assuming the version tag used in earlier-drafted code comments is still unclaimed.**

**Lesson 60 — For a densely-packed cluster of clickable hit-targets (Grit's 6 sub-nodes, ~75-95px apart), compute actual pairwise center-to-center distances before picking a uniform invisible hit-circle radius, rather than eyeballing it.** Sizing every hit-circle generously (e.g. +13px over the visible circle) looked safe in isolation but the tightest pair (Emotional Regulation ↔ Long-term Thinking, ~74px apart) would have overlapped. Computed all 15 pairwise distances for the 6-node cluster first, found the minimum, then picked a uniform radius (+3px over the visible circle) that keeps every pair's combined hit-circles under the tightest gap with margin to spare. **Lesson: "generous padding" on hit-targets is only safe when targets are far apart — for tightly packed clusters, do the actual distance math instead of guessing a radius that "feels" big enough.**

### 2026-07-03 — v2.24: Performance overhaul (CLS/INP/backdrop-filter/rAF)

**Lesson 54 — When a hover animation on one flex sibling has a measurable CLS score, the culprit is usually whichever *other* sibling it's pushing, not the animated element itself.** `#themeToggle` showed a CLS score of 0.2483 with nothing in its own CSS that should shift (fixed 32x32px box). Actual cause was `.nav-epic-standalone`, a sibling in the same flex row, animating `max-width: 0→160px` + `padding` on `#mainNav:hover` — a layout-triggering animation that pushed everything after it sideways, themeToggle included. Fix: keep the button's full box always reserved (space never changes) and animate only `opacity` + `transform: scale()`. **Lesson: when a CLS score is attributed to element X, check X's flex/grid siblings for layout-triggering animations before assuming X's own CSS is at fault — the measured element and the actual cause are often different nodes in the same row.**

**Lesson 55 — A popover/tooltip's `offsetWidth`/`offsetHeight` measured inside the hover handler is a forced synchronous reflow on every hover, even if the content never changes after being built.** Intel-pill popovers re-measured their own size every time `showPopover()` ran — added in v2.20's vertical-space-aware flip fix, which needed the popover's height to decide whether to flip above/below. But the popover's content is built once and never changes afterward, so its size doesn't either. Fixed by measuring once right after `buildPopoverContent()` and caching the result, reused on every subsequent hover. **Lesson: a layout read that's "necessary" the first time doesn't mean it's necessary every time — check whether the underlying content is static before leaving a `getBoundingClientRect`/`offsetWidth` call inside a hot path.**

**Lesson 56 — Chunking a large synchronous `innerHTML` build via `requestAnimationFrame` is safe to do incrementally (one group per frame) as long as the inserted elements start in the same hidden/default state the original all-at-once render left them in.** `_renderDrawerContent`'s episode card grid built every category column's full markup in one `.map().join('')` pass, causing the drawer's 288-350ms open-time INP block. Confirmed via reading `.ep-cat-column` CSS that columns default to `display:none` (only `.visible` shows them) before doing the chunking rewrite — meant inserting columns one `requestAnimationFrame` at a time via `insertAdjacentHTML('beforeend', ...)` couldn't cause a visible flash/flicker, since a column stays invisible until `filterDrawerCat` explicitly marks it visible regardless of when it entered the DOM. Tail logic (filter-row build, parallax init, sort-row show, scan reset) extracted into a shared completion function so it still runs exactly once, in the same order, after the last column — for both the populated and empty-state branches. **Lesson: before chunking any bulk render, check what the inserted elements' *default* CSS state is — if they're already hidden until something else reveals them, staggering their insertion across frames is behaviorally invisible to the user.**

**Lesson 57 — A purely decorative canvas `requestAnimationFrame` loop should be timestamp-throttled, not left to run at the display's native refresh rate.** The Corner constellation background animation had no fps cap, so it ran at up to 120fps on ProMotion Macs for content that's slow ambient drift — no part of the animation needs more than ~30fps to look smooth. Fixed with a `ts - lastDrawTs < FRAME_INTERVAL` early-return inside `draw()`, still using `requestAnimationFrame` for scheduling (not `setInterval`) so it still pauses correctly when the tab is backgrounded. **Lesson: default any decorative/ambient canvas loop to a capped fps (throttled inside the rAF callback) unless the motion is fast enough that the difference would actually be visible — most background animations aren't.**

### 2026-07-03 — Multi-phase build session (OG Map fix, extract.html tooling, Spark bug, Corner mobile)

**Lesson 51 — `animation: ... both` on an unconditional CSS class beats a `.hidden` class's `opacity:0`, even at higher specificity, for as long as fill-mode keeps the animation "in effect."** The Spark "Did it land?" row appeared almost instantly instead of after the intended 8s. Root cause: `.cs-feedback-row { animation: fadeIn 0.4s ease both; }` ran unconditionally on render, and the `both` fill-mode's final keyframe (`opacity:1`) sat above the normal cascade for that property — beating `.cs-post-prompt.cs-hidden { opacity:0 }` regardless of specificity. Fix: delete the redundant animation; the element already had a working `transition: opacity 0.3s ease` on `.cs-post-prompt` driven by the JS's `cs-hidden`/`cs-visible` toggle at the correct 8000ms mark. Lesson: if a hidden/visible class pattern isn't working, check for a competing unconditional `animation` on the same property before assuming the JS timer is wrong.

**Lesson 52 — When JS sets `.textContent` directly on a button in multiple places, don't fix mobile-only display via markup (child spans) — fix it with CSS `font-size:0` + `::after`.** Corner's submit button needed emoji-only on mobile, but `cornerSubmit()` and `exitCornerMode()` both call `submit.textContent = '...'` (two different strings, for default and loading states) in four places. A first attempt wrapped the label in `<span>` children toggled by media query — but any JS `textContent` write wipes child elements, so it would have silently reverted to plain text on every submit/reset. Fixed with a CSS-only approach instead: `font-size:0` collapses whatever text is currently in the button to zero visual width without touching the DOM, and a `::after { content: '🥊' }` pseudo-element supplies the icon — works regardless of what JS just set `textContent` to.

**Lesson 53 — Sticky/fixed sidebar panels that must never affect an existing max-width content column: use `position: fixed` + a viewport-width media query gate, not a grid/flex sibling.** `extract.html`'s new sticky rules panel had to sit outside `.page`'s 880px column without changing that column's layout at all, on any viewport. `position: fixed` sidesteps the question entirely (it's removed from document flow, so it can never push or shrink `.page`); the `@media (min-width: 1440px)` gate is purely cosmetic (avoids visual overlap on narrower viewports) and carries zero layout risk if it's ever wrong.
...
**Lesson 49 — Airtable PAT 403s recurred; this is now a confirmed pattern, not a one-off.** Rolling out `publish-approved.yml` (the GitHub Action replacing Make.com's APPROVED→publish step), Airtable returned `403 INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND` on a token that had the correct scopes and base access — nothing about the token config looked wrong. Spent time re-checking scopes/base access before trying the known fix. Fix: generated a brand-new Airtable Personal Access Token and rotated only the `AIRTABLE_API_KEY` GitHub secret to it — worked instantly, no other changes needed. This is the exact same failure mode as **Lesson 2, 2026-05-18** ("Airtable PATs can silently break... creating a fresh PAT fixed it instantly"), just in a different tool (GitHub Action secret vs. browser-tool token) more than a month later. **Lesson: if Airtable 403s with `INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND` and the scopes/base access look correct, skip the re-diagnosis — regenerate the PAT first, it has now fixed this twice.**

### 2026-07-02 — v2.20/v2.21: DNA pill — two separate bugs, easy to conflate

**Lesson 47 — "It doesn't show up" can be a positioning bug, a data bug, or both — diagnose them separately.** Gergely reported the DNA pill "doesn't work" on desktop. First pass (v2.20) found and fixed a real bug: `showPopover()` anchored the popover's bottom edge just above the pill with no vertical-space check, so episodes with many DNA categories (518, 519 — tall content) computed a top edge off-screen. Fixed with a flip-below/clamp-to-top fallback. But testing on collection 520 (a brand-new episode) still showed nothing — a *second*, unrelated bug: DNA content itself was empty. Root cause: DNA used to always be populated because it was originally computed **live** from the episode's actual concept cards (the same category tally the currently-hidden `.ep-drawer-mix` bar still computes in `_renderDrawerContent`/`_renderThemeDrawerContent`). A later backfill script (`generate-episode-intel.js`) started writing a precomputed `dna` object into `episode_meta.json` for older episodes, but nothing keeps that file in sync for new ones — so new episodes got `dna: {}` and sat empty until a manual backfill ran. Fix (v2.21): `_renderIntelRow()` now takes an optional live-tally fallback computed at both call sites, used only when the backfilled JSON has no data — so DNA can never be empty again without a backfill dependency, and already-backfilled episodes are untouched. **Lesson: when a UI element "used to always work" and a recent architecture change (adding a precomputed data source) is in the mix, check whether the old live computation got silently retired instead of kept as a fallback.**

**Commit message convention was being skipped — flagged by Gergely.** The CORS fix, `saveIntelToGitHub()` fix, DNA popover fix, and first Vocab redesign all shipped without version tags or changelog/roadmap entries, breaking the standing rule above ("Documentation ... auto, end of every session — no reminder needed"). Went back and added retroactive v2.20 changelog/roadmap entries covering that work. Going forward: every commit gets a `v[X.Y] - description` title per the Commit message format rule, and changelog.md/roadmap.md/build-journal.md get updated the same session, not deferred.
...
**Lesson 44 — A single-shot JSON completion with no scratchpad is the actual mechanism behind "quality drifts after N concepts."** With no `thinking` param, the model has nowhere to actually run a 7-point self-check across 20-40 concepts plus 6 intel fields before committing final JSON text, it can only claim to have checked. Fix: enabled extended thinking (`thinking: {type:'enabled', budget_tokens: 12000}`, `max_tokens` raised 20000 to 32000) on the main `runExtraction()` call in extract.html. Confirmed safe before enabling: response parsing already does `data.content?.find(b => b.type === 'text')`, so a new 'thinking' content block is silently ignored by existing code, no parser change needed. Also reframed the self-check instruction from "run before returning output" (implicitly: after drafting everything) to "apply while drafting each concept" and added an explicit quality-over-quantity line (40 is a ceiling, not a target) to both extract.html and extract-concepts.js.

**Lesson 45 — vocab_vault's stored count and its default preview count are two different numbers, don't conflate them.** Extraction now stores 20-25 vocab_vault entries per episode (raised from 5-7), but the UI still previews only the 5-7 best by default with an option to expand. This is deliberate (avoids re-running extraction if the preview count changes later) but means the 15 episodes that got intel generated before this change are stuck at 5-7 stored entries. **Action item, not yet done:** rerun `tools/generate-episode-intel.js` (or extract.html's intel panel) on those 15 episodes to backfill the full 20-25 list. See `docs/architecture.md` Episode Intelligence Layer section.

**Scope note:** `upload.html` and `SHORT_EXTRACTION_PROMPT` were explicitly left untouched this session (Gergely doesn't currently use either path); `api/extract-concepts.js` got the same rule-consistency fixes as extract.html but its long-term role is undecided pending the Make.com-vs-GitHub-Actions call (see `docs/pending-decisions.md`).
